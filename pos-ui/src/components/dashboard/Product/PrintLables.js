...
import { FaCog, FaInfo, FaInfoCircle, FaSearch } from 'react-icons/fa'
...
const PrintLables = () => {
    ...
    const onChangeInput = (e, index) => {
        ...
        const editData = inputData.map((item,index) =>
    ...
    const AddToArray = (name) => {
        ...
        let selingPriceGrp = ""
        let newdata =[]
        newdata[name] = { name, Lables, lotNumber, date, selingPriceGrp }
                    ...
                    <div className='flex  w-3/4 relative'>
                        ...
                        {isClicked &&
                            <ul
                                ...
                                className={`bg-white  w-3/4 mx-[30px] border-[1px] z-10 absolute top-8 border-gray-600  ${isClicked ? "max-h-60" : "max-h-0"} `}
                            >
                                ...
                                <tbody >
                                    {inputData.map((value, index) => {
                                        return <tr key={index} className={`${(index + 1) % 2 === 0 ? "bg-gray-200" : ""}`}>
                                            ...
                                            <td className=" py-1 px-1"><input type='number' name='Lables' value={value.Lables} onChange={(e)=>onChangeInput(e,index) } className='border-[1px] w-3/4 border-black focus:outline-none' /> </td>
                                            <td className="px-1 py-1 text-sm"><input name='lotNumber' type='number' value={value.lotNumber} onChange={(e)=>onChangeInput(e,index) } className='border-[1px] w-3/4 border-black focus:outline-none' /></td>
                                            <td className="px-1 py-1"> <input name='date' value={value.date} onChange={(e)=>onChangeInput(e,index) } type='datetime-local' className='border-[1px] w-full border-black focus:outline-none' /></td>
                                            <td className="px-1 py-1 text-sm">
                                                <select name='selingPriceGrp' value={value.selingPriceGrp} onChange={(e)=>onChangeInput(e,index) } className='border-[1px] w-3/4 border-black focus:outline-none'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-4 mt-5 gap-5'>
                    <div className='flex flex-col'>
                        <div className='flex items-center '>
                            <input value={formData.isProductName} onChange={(e)=> setformData({...formData, isProductName:e.target.value})}  checked={formData.isProductName} type='checkbox' className='h-4 w-4' />
                    ...
                    <div className='flex flex-col'>
                        <div className='flex items-center '>
                            <input value={formData.isProductVariation} onChange={(e)=> setformData({...formData, isProductVariation:e.target.value})} checked={formData.isProductVariation} type='checkbox' className='h-4 w-4' />
                    ...
                    <div className='flex flex-col'>
                        <div className='flex items-center '>
                            <input value={formData.isProductPrice} onChange={(e)=> setformData({...formData, isProductPrice:e.target.value})} checked={formData.isProductPrice} type='checkbox' className='h-4 w-4' />
                    ...
                    <div className='flex flex-col'>
                        <div className='flex items-center '>
                            <input value={formData.isBusinessName} onChange={(e)=> setformData({...formData, isBusinessName:e.target.value})} checked={formData.isBusinessName} type='checkbox' className='h-4 w-4' />
                    ...
                    <div className='flex flex-col'>
                        <div className='flex items-center '>
                            <input value={formData.isPrintPackingDate} onChange={(e)=> setformData({...formData, isPrintPackingDate:e.target.value})} checked={formData.isPrintPackingDate} type='checkbox' className='h-4 w-4' />
                    ...
                    <div className='flex flex-col'>
                        <div className='flex items-center '>
                            <input value={formData.isPrintLotNumber} onChange={(e)=> setformData({...formData, isPrintLotNumber:e.target.value})} checked={formData.isPrintLotNumber} type='checkbox' className='h-4 w-4' />
