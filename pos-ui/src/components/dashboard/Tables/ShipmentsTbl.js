                    ...
                    <tbody >
                        {record.map((value, index) => {
                            return <tr key={index} className=''>
                                ...
                                {col7 && <td className=" py-1 px-1">{value.Role}</td>}
                                {col8 && <td className="px-1 py-1 text-sm">
                                    <button onClick={() => { setIsCliked(true); setIseditship(true); setPaymentId(value.id); setDynWidth("50%") }} className='bg-green-500 text-white px-2 text-xs rounded-xl'>status</button>

                                </td>}
                                {col9 && <td className="px-1 py-1 text-sm">
                                    <button onClick={() => { setIsCliked(true); setIsShowPayment(true); setPaymentId(value.id); setDynWidth("50%") }} className='bg-green-300 text-white px-2 text-xs rounded-xl'>status</button>
