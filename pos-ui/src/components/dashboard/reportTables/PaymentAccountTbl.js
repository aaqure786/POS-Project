...
import LinkAccount from '../PaymentsAccounts/LinkAccount';
// import { Link } from 'react-router-dom';


const PaymentAccountTbl = () => {
    const dummyData = [
        {
            id: 1,
            Username: "username",
                    ...
                    <button onClick={() => { setColvis(!colvis) }} className='flex border-[1px] relative px-2 py-1 hover:bg-gray-400 border-gray-600 bg-gray-200 '>
                        ...
                        {colvis && <div className='absolute top-7 shadow-md shadow-gray-400 bg-white w-[150px]'>
                            <ul className='flex flex-col text-start px-2 justify-start'>
                                <li className={` w-full py-[1px] ${col1 ? "" : "bg-blue-600"} hover:bg-blue-400 `} onClick={() => { setCol1(!col1) }}>Date</li>
                                <li className={` w-full py-[1px] ${col2 ? "" : "bg-blue-600"} hover:bg-blue-400 `} onClick={() => { setCol2(!col2) }}>Payment Reference No.</li>
                                <li className={` w-full py-[1px] ${col3 ? "" : "bg-blue-600"} hover:bg-blue-400 `} onClick={() => { setCol3(!col3) }}>Invoice No./ Ref. No.</li>
                                <li className={` w-full py-[1px] ${col4 ? "" : "bg-blue-600"} hover:bg-blue-400 `} onClick={() => { setCol4(!col4) }}>Amount</li>
                                <li className={` w-full py-[1px] ${col5 ? "" : "bg-blue-600"} hover:bg-blue-400 `} onClick={() => { setCol5(!col5) }}>Payment Type</li>
                                <li className={` w-full py-[1px] ${col6 ? "" : "bg-blue-600"} hover:bg-blue-400 `} onClick={() => { setCol6(!col6) }}>Account</li>
                                <li className={` w-full py-[1px] ${col7 ? "" : "bg-blue-600"} hover:bg-blue-400 `} onClick={() => { setCol7(!col7) }}>Description</li>
                ...
                <table id='usertbl'  className="table-fixed w-full ">
                    <thead>
                        <tr className='h-[50px] bg-white text-sm px-1 text-center'>
                            {col1 && <th className=" py-2 w-10  border-l-[1px] border-r-[1px] border-gray-200  tracking-wider  text-gray-900 "> Date</th>}
                            {col2 && <th className=" py-2 title-font border-l-[1px] border-r-[1px] border-gray-200  tracking-wider font-medium text-gray-900 ">Payment Reference No.</th>}
                            {col3 && <th className=" py-2 title-font border-l-[1px] border-r-[1px] border-gray-200  tracking-wider font-medium text-gray-900 ">Invoice No./ Ref. No.</th>}
                            ...
                            {col5 && <th className=" py-2 title-font border-l-[1px] border-r-[1px] border-gray-200   tracking-wider font-medium text-gray-900 ">Payment Type</th>}
                            {col6 && <th className=" py-2 title-font border-l-[1px] border-r-[1px] border-gray-200   tracking-wider font-medium text-gray-900 ">Account</th>}
                            {col7 && <th className=" py-2 title-font border-l-[1px] border-r-[1px] border-gray-200  tracking-wider font-medium text-gray-900 ">Description</th>}
                        ...
                        {record.map((value, index) => {
                            return <tr key={index} className='text-sm'>
                                ...
                                {col8 && <td className="px-1 py-1"> 
                                    <button onClick={()=>{setIsClicked(true); setIsView(true); setViewId(value.id)}} className='bg-blue-500 text-white text-xs p-1 rounded-md'>Link Account</button>
                                </td>}
...
export default PaymentAccountTbl